...
def raster_preview(table, rid):
    try:
        colormap = request.args.get('colormap', 'viridis') # 'viridis', 'plasma', 'cividis', 'inferno'
        ...
        cur.execute(f"""
            SELECT ST_AsPNG(ST_ColorMap({raster_column}, 1, %s)),
        ...
        png_data, ymin, xmin, ymax, xmax = img_row
...
def get_attributes(table):
    ...
    try:
        ...
        """, (tables[table]['schema'], table, geom_col))
        columns = [row[0] for row in cur.fetchall()]
        if not columns:
            return jsonify({'error': f'No attributes found for {table}'}), 404

        columns_str = ', '.join(columns)
        query = f"SELECT {columns_str}, row_number() OVER () AS _rowid FROM {tables[table]['schema']}.{table} LIMIT 1000"
...
@app.route('/delete_raster/<table>', methods=['POST'])
