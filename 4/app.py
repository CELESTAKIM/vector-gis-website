...
# These configurations are used to connect to the PostGIS databases.
# IMPORTANT: You will need to change these credentials to match your setup.
RASTER_DB_CONFIG = {
    "host": "localhost",
    "dbname": "DEM",
    "user": "postgres",
...
# These helper functions establish connections to the raster and vector databases.
...
# Helper function to get the name of the raster column for a given table
...
# Helper function to retrieve all spatial tables from the vector database
...
# Helper function to assign a consistent color to each table based on its name
...
# @app.route("/")
# def index():
#     # Renders the main HTML template (frontend) which will be served separately by React.
#     # In this new setup, this route can be considered for a simple status check or API root.
...
# Retrieves a list of all raster tables and their bounding boxes
...
# Provides a PNG image preview of a specific raster tile, reclassified for visualization.
...
def raster_preview(table, rid):
    try:
        ...
        # Get summary stats (min/max) for reclassification
        cur.execute(f"""
        ...
        # Use ST_Reclass to scale the raster to an 8-bit unsigned integer (0-255) and then convert to PNG
        cur.execute(f"""
            SELECT encode(
                ST_AsPNG(
                    ST_Reclass(
...
# Downloads a specific raster tile as a GeoTIFF file.
...
# Retrieves a layer's data as GeoJSON for display on the map.
...
# Retrieves a layer's attribute data as a JSON object.
...
# Uploads a new vector layer from a zipped shapefile.
...
def upload_vector():
    ...
    try:
        ...
        gdf['row_number'] = range(len(gdf))  # Add row_number for selection
    except Exception as e:
        shutil.rmtree(temp_dir)
...
# Uploads a new raster layer from a zipped GeoTIFF.
...
# Downloads a selection of vector features as a zipped GeoJSON file.
...
def download_selected():
    ...
    try:
        conn = get_vector_connection()
        cur = conn.cursor()
        # Fallback to 'ctid' if no primary key is found, though 'row_number' is used in other places.
    ...
    try:
        # Use the 'row_number' column to select features
...
# Merges selected features from multiple layers into a single downloadable GeoJSON.
...
def merge_layers():
    ...
    try:
        ...
        for layer_info in layers:
            ...
            # Retrieve selected features based on row_number
            query = sql.SQL("""
                SELECT t1.*
                FROM {table} t1
...
# Deletes an entire vector layer from the database.
...
# Creates a new vector layer from selected features of an existing layer.
